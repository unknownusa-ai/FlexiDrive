class VehicleImageResolver {
  ...
  static String resolveFromVehicle(
    ...
    if (_isExternalOrLocalPath(vehicleImage)) return vehicleImage;
    if (_isExternalOrLocalPath(preferred)) return preferred;

    final nameMatchedAsset = assetByVehicleMap(vehicle);
    if (nameMatchedAsset != null) return nameMatchedAsset;

    if (preferred.isNotEmpty) return preferred;
    if (vehicleImage.isNotEmpty) return vehicleImage;
    return fallback;
  }
  ...
  static String? assetByVehicleName(String value) {
    ...
    if (normalized.contains('cx5')) return 'assets/imagenes_carros/cx5.jpg';
    if (normalized.contains('mazda3')) return 'assets/imagenes_carros/mazda3.jpg';
    if (normalized.contains('corolla')) return 'assets/imagenes_carros/corolla.jpg';
    if (normalized.contains('sandero')) {
      return 'assets/imagenes_carros/Renault-Sandero.jpg';
    }
    if (normalized.contains('onix')) return 'assets/imagenes_carros/onix.jpeg';
    if (normalized.contains('ranger')) return 'assets/imagenes_carros/ranger.jpg';
    if (normalized.contains('tesla')) return 'assets/imagenes_carros/tesla.jpg';
    if (normalized.contains('mercedes') || normalized.contains('benz')) {
      return 'assets/imagenes_carros/mercedes.jpg';
    }
    if (normalized.contains('porsche')) return 'assets/imagenes_carros/porsche.jpg';
    if (normalized.contains('audia4') ||
        (normalized.contains('audi') && normalized.contains('a4'))) {
      return 'assets/imagenes_carros/audia4.jpg';
    }
    if (normalized.contains('sentra')) return 'assets/imagenes_carros/sentra.jpg';
    if (normalized.contains('bmw')) return 'assets/imagenes_carros/bmw.jpg';
    if (normalized.contains('tucson')) return 'assets/imagenes_carros/tucson.jpg';
    if (normalized.contains('sportage')) return 'assets/imagenes_carros/sportage.jpg';
    return null;
  }
